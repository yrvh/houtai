  ...
  <main-card2 title1="权限管理" title2="角色管理">
    <div slot="content">
      <div class="role-top">
        <el-row :gutter="8">
          ...
          <el-col :span="2">
            <el-button type="danger" class="role-search role-button" @click="getCommunicateList">查询</el-button>
          </el-col>
          <el-col :span="2">
            <el-button type="info" class="role-clear role-button" @click="getCommunicateList">重置</el-button>
          </el-col>
        </el-row>
      </div>
    ...
    <el-dialog title="添加账户公司" :visible.sync="addDialogVisible" width="40%" @close="addDialogClosed">
      ...
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="90px">
        <el-form-item label="账户名称" prop="role_name">
          <el-input v-model="addForm.role_name"></el-input>
        </el-form-item>
        <el-form-item label="商户ID" prop="role_id">
    ...
    <el-dialog title="修改账户信息" :visible.sync="editDialogVisible" width="40%" @close="editDialogClosed">
      ...
      <span slot="footer" class="editDialog-footer">
        ...
        <el-button @click="editRole" type="primary" class="editDialog-footer-sure">确 定</el-button>
        <el-button @click="delRole" type="danger" plain class="editDialog-footer-del">删 除</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog title="注册账号" :visible.sync="regDialogVisible" width="40%" @close="regDialogClosed">
      ...
      <el-form :model="regForm" :rules="regFormRules" ref="regFormRef" label-width="90px">
        <el-form-item label="商户ID">
          <el-input v-model="regForm.role_id" disabled></el-input>
        </el-form-item>
        <el-form-item label="商户账号" prop="role_name">
          <el-input v-model="regForm.role_name"></el-input>
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button @click="regRole" type="primary">确 定</el-button>
      </span>
    </el-dialog>
  </main-card2>
</template>
...
<script>
  ...
  export default {
    ...
    created() {   // 生命周期函数, 用于初始化页面
      this.getRoleList()   // 调用该函数初始化账户的列表区域
    },


    methods: {
      async getRoleList() {   //获取账户列表
      ...
      addRole() {   // 点击确定按钮, 添加新账户
        this.$refs.addFormRef.validate(async (valid) => {
          if(!valid) return
          // 如果校验成功,, 可以发起网络请求. 来添加账户
          const {data:res} = await this.$axios({
            url:'/ponyproperty-manager/role/addrole',
            method: 'post',
            transformRequest: [function (data) {
              return Qs.stringify(data)
            }],
          ...
          this.getRoleList()   // 重新请求最新数据, 重新渲染页面
          console.log(res)
        })
      },
      ...
      editRole() {   // 点击确定按钮, 修改账户
        this.$refs.editFormRef.validate(async (valid) => {
          ...
          const {data:res} =await this.$axios({
            url:'/ponyproperty-manager/role/updaterole',
            method: 'post',
            transformRequest: [function (data) {
              return Qs.stringify(data)
            }],
      ...
      async delRole() {   // 删除一条商户, 触发的函数
        const {data:res} =await this.$axios({
          url:'/ponyproperty-manager/role/deleterole',
          method: 'post',
          transformRequest: [function (data) {
            return Qs.stringify(data)
          }],
          data: { role_id: this.editForm.role_id }
        })
      ...
      regRole() {   // 点击确定按钮, 注册账号
        this.$refs.regFormRef.validate(async (valid) => {
          ...
          const {data:res} =await this.$axios({
            ...
            data: {
              role_id: this.regForm.role_id,
              role_name: this.regForm.role_name,
...
<style scoped>
  .propertyrole-select { background-color: #FF6565; padding-left: 0 !important; }
  #ttt  {line-height: 40px; background-color: #F5FAFA; color: #999; font-size: 14px; border-right: 1px solid #d00; padding: 0 18px; }

  .role-add{ margin-top: 20px;}
  .role-add .el-button { background-color: #235FED !important; color: #fff;}
  .role-add .el-button:hover { background-color: #3A77FE !important;}
  ...
  .role-button { width: 70px; color: #fff; }
  .role-search { margin-left: 10px;}
</style>